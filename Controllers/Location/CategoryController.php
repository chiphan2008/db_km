...
class  CategoryController extends BaseController {
	...
	public function getContentByCategory($category, $request, $param){
		...
		if(session()->has('currentLocation') && !isset($param['country']) && !$request->ajax()){
		...
		$arrData['ads'] = PublishAds::select('publish_ads.*')
											...
											->where('type_ads.machine_name','quang_cao_trang_list')
		...
		$arrData['type_ads'] = TypeAds::where('machine_name','quang_cao_trang_list')->first();
		
		// dd(\DB::getQueryLog());
		if($request->ajax()){
			$arrReturn['currentPage'] = intval($request->page);
			$arrData['currentPage'] = intval($request->page);
			...
			$arrReturn['html'] = view('Location.category.category_list', [
				'contents'=>$arrData['contents'],
				'ads'=>$arrData['ads'],
				'type_ads'=>$arrData['type_ads'],
				'currentPage' => $arrData['currentPage']
			])->render();
			// echo json_encode($arrReturn);
			// dd($request->all());
			return response($arrReturn);
		}else{			  
			$this->view->content = view('Location.category.category',$arrData);
	...
	public function getContentByCategoryItem($category, $category_item, $request){
		...
		// dd($arrData);
		$this->view->content = view('Location.category.category_item',$arrData);
		return $this->setContent();
	}
