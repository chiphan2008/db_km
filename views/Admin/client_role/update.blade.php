...
@section('content')
  ...
  <form class="form-horizontal form-label-left" method="post" action="{{route('update_client_role',['id' => $role->id,'group_id'=>$group_id])}}" enctype="multipart/form-data">
    {{ csrf_field() }}
    @if ($errors->has('error'))
    ...
    <div class="form-group">
      ...
      <div class="col-md-6 col-sm-6 col-xs-12">
        <input type="text" id="name" name="name" class="form-control col-md-7 col-xs-12" value="{{ $role->name?$role->name:'' }}" >
      </div>
      @if ($errors->has('name'))
