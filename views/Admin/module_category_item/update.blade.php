...
@section('content')
  ...
  <form id="form-category" class="form-horizontal form-label-left" method="post" action="{{route('update_module_category_item',['module_id'=>$module_id,'category_id' => $category_id,'category_item_id' => $category_item->category_item_id])}}" enctype="multipart/form-data">
    {{ csrf_field() }}
    @if ($errors->has('error'))
    ...
    <div class="form-group">
      <label class="control-label col-md-3 col-sm-3 col-xs-12" for="category_item">{{trans('global.category')}} </label>
      <div class="col-md-6 col-sm-6 col-xs-12">
        <select class="form-control" name="category_item" id="category_item">
         @if (isset($list_category_item))
            @foreach ($list_category_item as $one_category)
              <option value="{{$one_category->id}}" {{ $category_item->category_item_id == $one_category->id ? 'selected' : '' }}>{{$one_category->name}}</option>
            @endforeach
          @endif
        </select>
      </div>
    ...
    <div class="form-group">
      ...
      <div class="col-md-6 col-sm-6 col-xs-12">
        {{trans('global.inactive')}} <input type="checkbox" class="js-switch" {{ $category_item->active == '1' ? 'checked' : '' }} id="active" name="active"> {{trans('global.active')}}
      </div>
    </div>
...
<script type="text/javascript">
  ...
  $(function(){
    ...
    var html_image = '<img src="{{$category_item->image?$category_item->image:'' }}" alt="Image preview" class="thumbnail" style="max-width: 250px; max-height: 250px">';
    $('.file-tab.image').prepend(html_image)

    var html_background = '<img src="{{$category_item->background?$category_item->background:'' }}" alt="Background preview" class="thumbnail" style="max-width: 250px; max-height: 250px">';
