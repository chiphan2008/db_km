    ...
    <div class="bg-gray">
      <div class="modal-dialog my-0 py-5">
        ...
        <div class="modal-body">
          <h3 class="title-form mr-0 text-uppercase text-center mb-3">THAY ĐỔI MẬT KHẨU</h3>
          <form class="form-sin" method="post" action="{{ url('client-password/reset') }}">
            ...
            @if(isset($email))
              <fieldset class="form-group{{ $errors->has('email') ? ' has-danger' : '' }}">
                <input id="email" type="email" class="form-control" name="email" value="{{$email}}" style="display: none" required placeholder="Email">
                @if ($errors->has('email'))
            ...
            <fieldset class="form-group {{ $errors->has('password_confirmation') ? ' has-danger' : '' }}">
              <input id="password-confirm" type="password" class="form-control" name="password_confirmation" placeholder="Lặp lại password" required>
              @if ($errors->has('password_confirmation'))
                <div class="form-control-feedback">{{ $errors->first('password_confirmation') }}</div>
              @endif
            </fieldset>
            <div class="form-group">
              <input type="submit" class="btn-sin btn btn-primary d-block" value="Đăng Nhập"/>
            </div>
          </form>
        </div>
      </div>
