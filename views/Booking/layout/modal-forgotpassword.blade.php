  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <h3 class="title-form mr-0 text-uppercase">{{strtoupper(trans(DS.'global.forgot_password'))}}</h3>
        <form class="form-sin" method="post" id="form-forgot" action="{{ url('client-password/email') }}">
          ...
          <fieldset class="form-group" id="fieldset-email-fg">
            <input type="email" class="form-control" id="email-fg" name="email" placeholder="Email">
            <div class="form-control-feedback" id="div-email-fg" style="display: none"></div>
          </fieldset>
          <label class="forgot-pass text-center px-5">{{trans(DS.'global.send_mail_forgot')}}.</label>
          <div class="form-group">
            <button id="send_mail_forgot"  type="submit" class="btn-sin btn btn-primary d-block">{{trans(DS.'global.send')}}</button>
          </div>
        </form>
      </div>
      <!-- modal-body -->
      ...
      $.ajax({
        ...
        success: function (data) {
          if(data.errors)
          {
            ...
            $('#div-email-fg').hide();
            $("#form-forgot .forgot-pass").css("color","#009926").text('{{trans('valid.email_has_sent')}}');
          }
        },
        error: function (data) {
          $("#fieldset-email-fg").addClass("has-danger");
          $('#div-email-fg').html('{{trans('valid.email_email')}}').show();
          $('#send_mail_forgot').prop('disabled', false);
        }
      });
    });
