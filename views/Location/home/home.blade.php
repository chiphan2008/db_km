			...
			<div class="content-banner">
				 <div class="wrap-circle-cate">
					<div class="circle-logo  circle">
						...
						<span>{{trans('Location'.DS.'layout.others')}}</span>
					</div>
							...
							@endif
								 <div class="circle-cate-item circle">
									 ...
									 {{-- @else --}}
										 ...
										 <a href="{{url('/search2')}}?category={{$categories[$i]->id}}" title="{{$categories[$i]->name}}">
									 {{-- @endif --}}
...
<script type="text/javascript" charset="utf-8">
     ...
     *
     */
    var redundancy = $(window).height() - $("#footer").height() - $("#header").height();
 		$('.banner-home').css("height", redundancy);
	...
	function animatePoint(){
		...
		for (var i = 0; i < circle.length; i++) {
			...
			if($(window).width()>=768){
				if($(window).width()==768){
					var point = findNewPoint(x1, y1, Ang, 150);
				}else{
