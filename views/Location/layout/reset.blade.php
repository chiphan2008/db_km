        ...
        <div class="modal-body">
          ...
          <form class="form-sin" method="post" action="{{ url('client-password/reset') }}">
            ...
            <fieldset class="form-group{{ $errors->has('password') ? ' has-danger' : '' }}">
              <input id="password" type="password" class="form-control" name="password" placeholder="{{trans('Location'.DS.'layout.pwd')}}" required>
              @if ($errors->has('password'))
                <div class="form-control-feedback">{{ $errors->first('password') }}</div>
              @endif
            </fieldset>
            <fieldset class="form-group {{ $errors->has('password_confirmation') ? ' has-danger' : '' }}">
              <input id="password-confirm" type="password" class="form-control" name="password_confirmation" placeholder="{{trans('Location'.DS.'layout.re_pwd')}}" required>
              @if ($errors->has('password_confirmation'))
                <div class="form-control-feedback">{{ $errors->first('password_confirmation') }}</div>
              @endif
            </fieldset>
            <div class="form-group">
              <input type="submit" class="btn-sin btn btn-primary d-block" value="{{trans('global.update')}}"/>
            </div>
          </form>
        </div>
      </div>
