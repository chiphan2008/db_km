	...
	<script>
		...
		function loadContentSearch(loadNew, loadMore){
			...
			if(max_page){
				...
				if($(".content-search-page input[name='q']").val()){
					query.push({name:'q',value:$(".content-search-page input[name='q']").val()});
				}
				...
				query.forEach(function(obj,key){
					if(obj.value != "" && obj['name'] !='_token' && obj['name'] !='page'){
						if(obj['name']=='q'){
							url.push({name:'q',value:$(".content-search-page input[name='q']").val()});
						}else{
							url.push(obj)
						}
