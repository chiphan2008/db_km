<div class="talk" id="comment{{$comment->id}}">
	...
	<div class="talk-header-right d-flex align-items-center hidden-md-up">
			<div onclick="likeComment(this)" class="talk-total total-like mx-2 {{count($comment->_has_liked)?'active':''}}" data-id-comment="{{$comment->id}}">
					<i class="icon-heart-empty"></i>
					<span>{{$comment->like_comment}}</span> {{trans('Location'.DS.'content.like')}}
			</div>
			...
			<div class="align-items-sm-center">
					...
					<form class="form-commit w-100 clearfix" data-type="reply" data-id-comment="{{$comment->id}}">
							<div class="emoji-picker-container">
									<input type="hidden" name="content_id" value="{{$comment->model_id}}">
	...
	<div class="talk-sub">
		@foreach($comment->_replies as $reply)
			@include('Location.content.comment_item_sub_render')
		@endforeach
	</div>
	@if($comment->_all_replies->count()>5)
	<div class="talk-loadmore text-center mt-3">
		<button data-page="2" data-total="{{$comment->_all_replies->count()}}" data-current="5" data-id-comment="{{$comment->id}}" data-id-content="{{$comment->model_id}}" onclick="loadMoreComment(this)" class="btn btn-primary" href="" title="">{{trans('global.view_more')}}</button>
	</div>
	@endif
	@endif
	<!-- end sub -->
