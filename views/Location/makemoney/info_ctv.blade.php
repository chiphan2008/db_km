	...
	<div class="row">
		<div class="col-md-12 mb-3">
			...
			<div class="info-name align-middle">
				<span class="text-uppercase  text-truncates ml-2 h5">{{$ctv->full_name?$ctv->full_name:''}}</span><br/>
				@if($ctv->address)
				<!-- <span class="follow  text-truncates ml-2">{{$ctv->address?$ctv->address:''}}&nbsp;</span><br/> -->
				@endif
				@if($ctv->email)
				<span class="follow  text-truncates ml-2">{{$ctv->email?$ctv->email:''}}&nbsp;</span><br/>
				@endif
				@if($ctv->phone)
				<span class="follow  text-truncates ml-2">{{$ctv->phone?$ctv->phone:''}}&nbsp;</span><br/>
				@endif
			</div>
		</div>
		<div class="col-md-12 mb-3">
			<h3>{{trans('Location'.DS.'makemoney.manager')}}:
				<a href="#" class="text-danger">{{$ctv->_daily->full_name}}</a>
			</h3>
		</div>
		<div class="col-md-12 mb-3">
			@php
		...
		<div class="col-md-12 mb-3">
			<h3>{{trans('Location'.DS.'makemoney.count_location')}}:
				<a href="#" class="text-danger">{{money_number($count_location_ctv)}}</a>
			</h3>
		</div>


		<div class="col-md-12 mb-3">
			<h3 data-toggle="collapse" href="#area" role="button" aria-expanded="false" aria-controls="area" >{{trans('Location'.DS.'makemoney.location')}}: <t class="text-danger">{{count($ctv->_area)}}</t> <i class="indicator fa fa-angle-right"></i></h3>
			<div class="collapse" id="area">
				<div class="row">
					@foreach($ctv->_area as $district)
