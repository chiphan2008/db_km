<div class="content-edit-profile-manager">
	...
	<div class="list-content-profile  ">
					...
					<li class="col-lg-4 col-6">
						<div class="card-vertical card">
							<div class="card-img-top">
								@if($content->moderation == 'publish' || $content->moderation == 'request_publish')
							...
							<div class="card-block py-2 px-0">
								<div class="card-description">
									<h6 class="card-title ">
										@if($content->moderation == 'publish' || $content->moderation == 'request_publish')
											<a href="{{LOCATION_URL}}/{{$content->alias}}" title="">{{$content->name}}</a>
										@else
											{{$content->name}}
										@endif
									</h6>
		...
		<div id="modal-submit-payment" class="modal modal-vertical-middle fade modal-submit-payment  modal-vertical-middle modal-report" tabindex="-1" role="dialog" aria-labelledby="modal-submit-payment modal-vertical-middle modal-vertical-middle" aria-hidden="true">
		...
		<div id="modal-submit-push" class="modal modal-vertical-middle fade modal-submit-payment  modal-vertical-middle modal-report" tabindex="-1" role="dialog" aria-labelledby="modal-submit-payment modal-vertical-middle modal-vertical-middle" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content text-center">
					<div class="modal-logo pt-4 text-center">
						<img src="/frontend/assets/img/logo/logo-large.svg" alt="">
		...
		<div class="modal modal-vertical-middle fade   modal-report" id="modal-notification" tabindex="-1" role="dialog" aria-labelledby="modal-notification" aria-hidden="true">
			<div class="modal-dialog modal-md" style="max-width: 540px;">
				<div class="modal-content p-4">
					...
					<form id="popupNotifyContent" class="form-notification-locaiton" method="post" action="{{route('postPopupNotifyContent')}}">
						...
						<!-- end  report modal modal-vertical-middle -->
					</form>
					<!-- end  form nitification location -->
				</div>
				<!-- end  modal modal-vertical-middle content -->
			</div>
		</div>

		<div id="modal-submit-ad" class="modal modal-vertical-middle fade modal-submit-payment  modal-vertical-middle modal-report" tabindex="-1" role="dialog" aria-labelledby="modal-notification" aria-hidden="true">
...
@section('JS')
	...
	<script type="text/javascript">
		...
		{
			$('#name_delete').html("{{trans('Location'.DS.'user.delete_loc_confirm')}}:<br/> " + name);
				...
				$.ajax({
					...
					success : function(response){
						...
						if(response.error==1){
							...
							toastr.success(response.message,"",{ timeOut: 9500 });
