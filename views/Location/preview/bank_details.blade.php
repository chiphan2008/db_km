            ...
            <div class="meta-post d-flex align-items-center hidden-sm-down">
              ...
              <div class="rating d-flex align-items-center" data-vote="0">
                <div class="star hidden">
                  <span class="full {{(isset($vote_point) && $vote_point>=1) ? 'star-colour' : ''}}" data-value="1"></span>
                  <span class="half {{(isset($vote_point) && $vote_point>=0.5) ? 'star-colour' : ''}}" data-value="0.5"></span>
                </div>
                <div class="star">
                  <span class="full {{(isset($vote_point) && $vote_point>=2) ? 'star-colour' : ''}}" data-value="2"></span>
                  <span class="half {{(isset($vote_point) && $vote_point>=1.5) ? 'star-colour' : ''}}" data-value="1.5"></span>
                  <span class="selected"></span>
                </div>
                <div class="star">
                  <span class="full {{(isset($vote_point) && $vote_point>=3) ? 'star-colour' : ''}}" data-value="3"></span>
                  <span class="half {{(isset($vote_point) && $vote_point>=2.5) ? 'star-colour' : ''}}" data-value="2.5"></span>
                  <span class="selected"></span>
                </div>
                <div class="star">
                  <span class="full {{(isset($vote_point) && $vote_point>=4) ? 'star-colour' : ''}}" data-value="4"></span>
                  <span class="half {{(isset($vote_point) && $vote_point>=3.5) ? 'star-colour' : ''}}" data-value="3.5"></span>
                  <span class="selected"></span>
                </div>
                <div class="star">
                  <span class="full {{(isset($vote_point) && $vote_point>=5) ? 'star-colour' : ''}}" data-value="5"></span>
                  <span class="half {{(isset($vote_point) && $vote_point>=4.5) ? 'star-colour' : ''}}" data-value="4.5"></span>
                  <span class="selected"></span>
                </div>
    ...
    <section class="my-4 my-md-5">
      <div class="container">
        <div class="header-style1 header mb-3">
          <h3>{{trans('Location'.DS.'preview.location_near_you')}}</h3>
        </div>
        <div id="map-2"></div>
      </div>
    </section>
  ...
  <div id="modal-notify-content" class="modal fade   modal-report show modal-animation" data-backdrop="false"  tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-md">
      <div class="modal-content p-4">
        ...
        <!-- end logo -->
        <h4 class="text-uppercase text-center">{{trans('Location'.DS.'preview.notification')}}</h4>
        <hr>
  ...
  @section('JS')
    ...
    <script>
        ...
        if (id_user === undefined) {
          ...
          $.ajax({
            ...
            success: function (data) {
              if (data.mess == true) {
                $('.point_like').text('(' + data.value + ')');
                if ($('.add-like i').hasClass('icon-heart-empty')) {
                  $('.add-like i').removeClass('icon-heart-empty').addClass('icon-heart')
                } else {
                  $('.add-like i').removeClass('icon-heart').addClass('icon-heart-empty')
                }
              }
            }
          })
      ...
      function calculateAverage(id_content,id_user,point) {
        ...
        $.ajax({
          type: "POST",
          data: {id_content: id_content, id_user: id_user, point: point, _token: $('meta[name="_token"]').attr('content')},
