...
@section('content')
	...
	<form class="form-horizontal form-label-left" method="post" action="{{route('update_option',['id'=>$option->id])}}" enctype="multipart/form-data">
		{{ csrf_field() }}
		@if ($errors->has('error'))
		...
		<div class="form-group">
			...
			<div class="col-md-6 col-sm-6 col-xs-12">
				<input type="text" id="name" name="name" class="form-control col-md-7 col-xs-12" value="{{ $option->name }}" >
			</div>
			@if ($errors->has('name'))
      ...
      <div class="col-md-6 col-sm-6 col-xs-12">
        <input type="number" min="0" step="1" id="weight" name="weight" class="order form-control col-md-7 col-xs-12" value="{{ $option->weight?$option->weight:0 }}" >
      </div>
      ...
      @endif
		...
		<div class="form-group">
			...
			<div class="col-md-6 col-sm-6 col-xs-12">
				{{trans('global.inactive')}} <input type="checkbox" class="js-switch" {{ $option->active == '1' ? 'checked' : '' }} name="active"> {{trans('global.active')}}
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-md-6 col-sm-6 col-xs-12">
				<input type="checkbox" onclick="showCancel()" id="extra"  name="extra" {{ $option->extra?'checked':'' }}>
			</div>
			@if ($errors->has('extra'))
		...
		<div class="form-group {{!$option->extra?'cancel':'no_cancel'}}">
			<label class="control-label col-md-3 col-sm-3 col-xs-12" for="price_extra">{{trans('Admin'.DS.'option.price_extra')}} <span
					class="required">*</span>
			</label>
			<div class="col-md-6 col-sm-6 col-xs-12">
				<input type="number" min="0" id="price_extra" name="price_extra" class="form-control col-md-7 col-xs-12" value="{{ $option->price_extra?$option->price_extra:0 }}">
			</div>
		...
		<div class="form-group">
			<div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
				<button type="submit" class="btn btn-success">{{trans('Admin'.DS.'option.update_option')}}</button>
			</div>
		</div>
	</form>
@endsection
