  ...
  <div class="row">
    <div class="col-md-12 col-sm-12 col-xs-12">
      <div class="x_panel">
        ...
        <div class="x_content">
          <br>
          <form id="form-update-content" method="post" action="{{route('update_bank_content',['id'=>$content->id])}}"
            ...
            <div class="form-group">
              ...
              <div class="col-md-6 col-sm-6 col-xs-12">
                BANK <input type="checkbox" class="js-switch"
                            {{ $content->extra_type == 'ATM' ? 'checked' : '' }} name="bank_type" {{ $content->extra_type == '' ? '' : 'disabled' }}>
                ATM
              </div>
            </div>
              ...
              @if($content->_date_open)
                ...
                @foreach($content->_date_open as $date)
                  <div class="col-md-6 col-sm-6 col-xs-12 {{$i>0?'col-md-offset-3 item_custom_open':''}}">
                    ...
                    <div class="col-md-3">
                      <input class="form-control choose_hour" type="text" name="date_open[{{$i}}][from_hour]"
                             value="{{$date->open_from}}" required="">
                    </div>
                    <div class="col-md-3">
                      <input class="form-control choose_hour" type="text" name="date_open[{{$i}}][to_hour]"
                             value="{{$date->open_to}}" required="">
                    </div>
                    @if($i>0)
                      ...
                      <div class="checkbox">
                        <label style="padding-left: 0px;">
                          <input type="checkbox" class="flat" name="service[]"
                                 value="{{$value->id_service_item}}" {{in_array($value->id_service_item, $data['list_service_content']) ? 'checked':''}}> {{$value->_service_item->name}}
                        </label>
                      </div>
                    </div>
                  @endforeach
              ...
              <div class="col-md-9 col-sm-9 col-xs-12">
                @if($data['role_user'] < 4)
                  @foreach($data['list_image_space'] as $src)
                    <div class="col-md-1 col-sm-1 col-xs-12" style="width: 110px; padding-left: 0px;"
                         id="image_spaces_{{$src['id']}}">
                      <a data-fancybox="image_space" href="{{asset($src['name'])}}">
                        <img style="height: 90px; width: 90px; border: 1px solid #000; margin: 2px"
                             src="{{$src['name']}}">
                      </a>
                      <span
                        style="background-color: rgb(35, 179, 119);cursor: pointer;color: white;box-shadow: 2px 2px 7px rgb(74, 72, 72);position: absolute;left: 77px;"
                        id="image_spaces_{{$src['id']}}"
                        onClick="deleteImage('{{$src['id']}}','image_spaces')">[X]</span>
                    </div>
                  @endforeach
                @else
                ...
                <div class="form-group">
                  ...
                  <div class="col-md-3 col-sm-3 col-xs-12">
                    <select class="form-control" name="moderation" id="moderation">
                      ...
                      <option value="request_publish" {{$content->moderation == 'request_publish' ? 'selected' : ''}}>
                        {{trans('Admin'.DS.'hotel.request_publish')}}</option>
                      <option value="reject_publish" {{$content->moderation == 'reject_publish' ? 'selected' : ''}}>
                        {{trans('Admin'.DS.'hotel.reject_publish')}}</option>
